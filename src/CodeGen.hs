...
transMips instr strLit fltLit = do fillData strLit fltLit
    ...
    where strBuf          = ["string_buffer: .space 1024"]
          ...
          strCompareEq    = ["    str_compare_eq:\nlb $a2, 0($a0)\nlb $a3, 0($a1)\naddi $a0, $a0, 1\naddi $a1, $a1, 1\nbne $a2, $a3, str_compare_eq_not_equal\nbne $a2, $0, str_compare_eq\nli $v0, 1\nj str_compare_eq_end\n    str_compare_eq_not_equal:\nli $v0, 0\n    str_compare_eq_end:\njr $ra\n"]
...
transIR ((IR.TOSTR t t1 t2):remainder) = do t1' <- getLocation t1 "String"
                                            ...
                                            let instrExec = case (t, t1', t2') of
                                                                               (_, Stack _, Stack _) -> ["lw $a0, " ++ t2'' ++ "($fp)"] ++ ["jal itos"] ++ ["sw " ++ t1'' ++ "($fp)"]
                                                                               (_, Stack _, RegI _)  -> ["lw $a0, " ++ t2'' ++ "($fp)"] ++ ["jal itos"] ++ ["move " ++ t1'' ++ ", $v0"]
                                                                               (_, RegI _, Stack _)  -> ["move $a0, " ++ t2''] ++ ["jal itos"] ++ ["sw " ++ t1'' ++ "($fp)"]
                                                                               ...
                                                                               ("Float", RegI _, Stack _)    -> ["mov.s $f12, " ++ t2''] ++ ["jal ftos"] ++ ["sw " ++ t1'' ++ "($fp)"]
